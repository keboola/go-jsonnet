...
type value interface {
	...
	// generatedByNativeFuncs is set if the value has been generated by a native function. See Notifier interface.
	generatedByNativeFunctions() []nativeFnRef
	markGeneratedByNativeFunction(fn *NativeFunction, args []interface{}, out value, partial bool)
...
type nativeFnRef struct {
	fn      *NativeFunction
	args    []interface{}
	out     value
	partial bool // true if the objects were merged, and therefore only part of the value is from the native function
}

type valueBase struct {
	// generatedBy is set if the value has been generated by a native function.
	// See Notifier interface.
	generatedBy []nativeFnRef
}

func (v *valueBase) aValue() {}

func (v *valueBase) generatedByNativeFunctions() []nativeFnRef {
	return v.generatedBy
}

func (v *valueBase) markGeneratedByNativeFunction(fn *NativeFunction, args []interface{}, out value, partial bool) {
	v.generatedBy = append(v.generatedBy, nativeFnRef{fn: fn, args: args, out: out, partial: partial})
}
...
func makeValueExtendedObject(left, right *valueObject) *valueObject {
	...
	// Copy the value history, mark records as partial because objects are merged.
	for _, fn := range left.generatedByNativeFunctions() {
		v.markGeneratedByNativeFunction(fn.fn, fn.args, fn.out, true)
	}
	for _, fn := range right.generatedByNativeFunctions() {
		v.markGeneratedByNativeFunction(fn.fn, fn.args, fn.out, true)
	}
